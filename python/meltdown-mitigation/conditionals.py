...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentagem = (power / theoretical_max_power) * 100

    if percentagem >= 80:
        return "green"
    if percentagem < 80 and percentagem >= 60:
        return "orange"
    if percentagem < 60 and percentagem >= 30:
        return "red"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if data < (threshold * (90 / 100)):
        return "LOW"

    if data < (threshold * 1.1):
        return "NORMAL"
