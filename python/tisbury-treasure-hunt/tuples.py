...
def convert_coordinate(coordinate):
    ...
    """
    return tuple((cord for cord in coordinate))
...
def compare_records(azara_record, rui_record):
    ...
    _, coordinate_azara = azara_record
    _, coordinates, _ = rui_record
    return coordinate_azara == "".join(coordinates)
...
def clean_up(combined_record_group):
    ...
    for record_group in combined_record_group:
        cleaned = record_group[:1] + record_group[2:]
        report += f"{cleaned}\n"
